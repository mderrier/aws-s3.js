...
function compare(files, objects, options) {
	...
	// compare local against remote, to find local files to upload (newest)
	for (let key of files.keys()) {
	  ...
	  // compare local against remote, to find local files to upload (updated)
	for (let key of files.keys()) {
		let file = files.get(key);
		let object = objects.get(key);
	...
	// Sort updated by reverse path length
	uploads = reverse(sortBy(uploads, [function(o) {return o.length}]))
	// merge newest and updated (newest first)
	uploads = newest.concat(uploads)
