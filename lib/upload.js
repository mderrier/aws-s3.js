...
const getGlobIgnore = require('./utils/ignore.js').getGlobIgnore;
...
const S3_METHODS = ['listObjectsV2', 'upload', 'putObject'];
...
function upload(options) {
  ...
  // start by ensuring that we have a list of files to work on
    ...
    .then(results => {
		...
		task.emit('queuing', (plan.uploads.length + plan.deletes.length))
