...
const fsFilesToMap = require('./file-maps').fsFilesToMap;
const glob = require('./utils/glob').glob;
const listObjects = require('./list-objects').listObjects;
const s3ContentsToMap = require('./file-maps').s3ContentsToMap;
...
function upload (options) {
  ...
  task.promise = getFilePaths(options)
    ...
    .then(() => Promise.all([
      listObjects(options.s3).then(s3ContentsToMap),
      fsFilesToMap(null, options.cwd, task.filePaths)
    ]));
    // .then((maps) => {
    //   const objects = maps[0];
    //   const files = maps[1];
    // });
